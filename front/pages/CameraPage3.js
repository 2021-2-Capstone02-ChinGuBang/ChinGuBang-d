...
export default function CameraPage3({navigation}) {
	...
	takeImage = async () => {
		...
		if (!hasPermission) {
			...
			if (!image.cancelled) {
        navigation.navigate('방 내놓기',{"image3":image.uri, "base3":image.base64})
    ...
    pickImage = async () => {
        ...
        if (!hasPermission) {
           ...
           if (!pickimage.cancelled) {
            navigation.navigate('방 내놓기',{"image3":pickimage.uri, "base3":pickimage.base64})
